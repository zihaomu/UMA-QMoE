#!/usr/bin/env python3
"""Build a fixed Qwen mixed-precision TargetPack from a frozen policy."""
...
def main() -> int:
    ...
    if args.manifest_output.exists():
        raise SystemExit("refusing to overwrite existing TargetPack manifest")
    ...
    if (
        ...
        raise RuntimeError("TargetPack requires the fixed BF16 Qwen manifest")
    ...
    policy = json.loads(args.policy_evidence.read_text(encoding="utf-8"))
    if (
        policy.get("kind") != "qwen_precision_policy"
        or policy.get("status") != "candidate"
        or policy.get("model", {}).get("model_id") != QWEN1_5_MOE.model_id
        or policy.get("model", {}).get("model_revision")
        != QWEN1_5_MOE.model_revision
        or policy.get("model", {}).get("model_manifest_sha256")
        != manifest_sha256
    ):
        raise RuntimeError("Qwen precision policy identity is incompatible")
    try:
        layer_encodings = {
            int(layer): encoding
            for layer, encoding in policy["layer_encodings"].items()
        }
    except (KeyError, AttributeError, TypeError, ValueError) as exc:
        raise RuntimeError("Qwen precision policy layer map is invalid") from exc
    ...
    manifest = write_target_pack(
        ...
        policy_evidence_sha256=canonical_sha256(policy),
