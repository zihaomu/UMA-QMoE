#!/usr/bin/env python3
"""Capture the fixed three-level Qwen1.5-MoE numerical Oracle streams."""
...
def main() -> int:
    ...
    if observed_backend != args.backend:
        raise RuntimeError(
            f"Oracle backend mismatch: requested {args.backend}, "
    ...
    if getattr(config, "moe_intermediate_size", None) != (
    ...
    hidden_size = QWEN1_5_MOE.hidden_size
    expert_fixture = (
    ...
    def moe_hook(_module: Any, _inputs: Any, result: Any) -> None:
        ...
        if not isinstance(value, torch.Tensor):
            raise RuntimeError("Qwen MoE block returned an unexpected value")
        moe_output["value"] = value.detach().cpu()
    ...
    try:
        ...
        for handle in handles:
            handle.remove()
    expected_layers = set(range(QWEN1_5_MOE.num_layers))
    if set(router_logits) != expected_layers or set(router_topk) != expected_layers:
        raise RuntimeError("Oracle capture did not observe every Qwen router")
    ...
    metadata = {
        "schema_version": 1,
        "kind": "qwen1_5_moe_reference_capture_metadata",
