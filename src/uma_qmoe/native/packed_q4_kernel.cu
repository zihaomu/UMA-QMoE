...
__global__ void packed_q8_bf16_gemv_kernel(
    const c10::BFloat16* __restrict__ input,
    const std::int8_t* __restrict__ quantized,
    const float* __restrict__ scales,
    c10::BFloat16* __restrict__ output,
    std::int64_t rows,
    std::int64_t output_features,
  ...
  for (std::int64_t column = threadIdx.x;
       column < input_features; column += blockDim.x) {
    const std::int64_t weight_index = weight_row + column;
    partial += static_cast<float>(input[input_row + column]) *
               static_cast<float>(quantized[weight_index]) *
               scales[weight_index / group_size];
  }
...
torch::Tensor uma_qmoe_q8_linear_cuda(
  ...
  const dim3 grid(
      ...
      1);
  const auto stream = at::cuda::getCurrentCUDAStream(input.get_device());
  packed_q8_bf16_gemv_kernel<<<grid, kThreads, 0, stream.stream()>>>(
      input.data_ptr<c10::BFloat16>(),
      quantized.data_ptr<std::int8_t>(),
      scales.data_ptr<float>(),
      output.data_ptr<c10::BFloat16>(),
      input.size(0),
      output_features,
